...
use rustree::dtl::{count_extant_genes, simulate_dtl};
...
fn test_bd_snapshot_seed_42() {
    ...
    assert_eq!(
        extant_leaves, 10,
        "Should have exactly 10 extant leaves for n=10"
    );
    assert!(all_leaves >= 10, "Total leaves >= extant leaves");
    assert!(
        events.iter().any(|e| e.event_type == BDEvent::Speciation),
        "Should have speciation events"
    );
    ...
    assert_eq!(
        tree.nodes.len(),
        tree2.nodes.len(),
        "Same seed must produce same tree"
    );
    assert_eq!(
        events.len(),
        events2.len(),
...
fn test_bd_pure_birth_snapshot() {
    ...
    let extinction_count = events
        .iter()
        .filter(|e| e.event_type == BDEvent::Extinction)
        .count();
...
fn test_dtl_snapshot_seed_42() {
    ...
    assert_eq!(
        ...
        count_extant_genes(&rec_tree2)
    );

    // Basic sanity checks
    assert!(
        !rec_tree.gene_tree.nodes.is_empty(),
        "Gene tree should have some nodes"
    );
}
