...
fn run_spr_test(moved_node_name: &str, receiver_name: &str, expected_filename: &str) {
    ...
    println!(
        "Original tree (Root: {}):",
        flat_tree.nodes[flat_tree.root].name
    );
    ...
    let donor_index = flat_tree
        .find_node_index(moved_node_name) // Use the helper method
        .unwrap_or_else(|| panic!("Donor node '{}' not found", moved_node_name));
    let receiver_index = flat_tree
        .find_node_index(receiver_name) // Use the helper method
        .unwrap_or_else(|| panic!("Receiver node '{}' not found", receiver_name));

    println!(
        "Performing SPR: Donor='{}'({}), Recipient='{}'({})",
    ...
    println!(
        "Modified tree (Root: {}):",
        flat_tree.nodes[flat_tree.root].name
    ); // Print root name from flat_tree.root
    ...
    match compare_nodes_topology(&computed_tree, &expected_tree) {
        ...
        Ok(false) => {
            ...
            let computed_newick = computed_tree
                .to_newick()
                .unwrap_or_else(|e| format!("<error: {}>", e));
            let expected_newick = expected_tree
                .to_newick()
                .unwrap_or_else(|e| format!("<error: {}>", e));
