...
fn benchmark_dtl_varying_rates() {
    ...
    let (mut species_tree, _) = simulate_bd_tree(10, 1.0, 0.3, &mut rng);
...
fn benchmark_dtl_varying_species_tree_size() {
    ...
    for &n_species in &sizes {
        ...
        for _ in 0..n_reps {
            ...
            let (mut species_tree, _) = simulate_bd_tree(n_species, 1.0, 0.3, &mut rng);
            species_tree.assign_depths();
...
fn benchmark_dtl_scalability() {
    ...
    let (mut species_tree, _) = simulate_bd_tree(20, 1.0, 0.3, &mut rng);
    species_tree.assign_depths();
...
fn benchmark_dtl_transfer_intensity() {
    ...
    let (mut species_tree, _) = simulate_bd_tree(15, 1.0, 0.3, &mut rng);
    species_tree.assign_depths();
