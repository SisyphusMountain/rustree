...
pub fn save_bd_events_to_csv(events: &[TreeEvent], filename: &str) -> io::Result<()> {
...
pub fn save_dtl_events_to_csv(events: &[DTLEvent], species_tree: &FlatTree, filename: &str) -> io::Result<()> {
    let file = File::create(filename)?;
    let mut writer = BufWriter::new(file);
    writeln!(writer, "{}", DTLEvent::csv_header())?;
    for event in events {
        writeln!(writer, "{}", event.to_csv_row(species_tree))?;
    }
    writer.flush()?;
    Ok(())
}
