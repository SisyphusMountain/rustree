...
///
/// This is the core implementation for the per-gene-copy DTL model.
...
///
/// // Single tree:
/// let (tree, events) = simulate_dtl_iter(&species_tree, 0, 0.2, 0.2, 0.1, None, None, 1, true, &mut rng)
///     .unwrap()
///     .single().unwrap();
///
/// // Collect into vectors:
/// let (trees, events) = simulate_dtl_iter(&species_tree, 0, 0.2, 0.2, 0.1, None, None, 100, true, &mut rng)
///     .unwrap()
///     .collect_all().unwrap();
/// ```
pub fn simulate_dtl_iter<'a, R: Rng>(
    ...
    simulate_dtl_iter_with_config(species_tree, origin_species, config, n_simulations, require_extant, rng)
}

/// Core implementation for creating a per-gene DTL simulation iterator from a [`DTLConfig`].
pub fn simulate_dtl_iter_with_config<'a, R: Rng>(
    species_tree: &FlatTree,
    origin_species: usize,
    config: DTLConfig,
    n_simulations: usize,
