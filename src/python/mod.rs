...
pub use species_tree::{PySpeciesNode, PySpeciesTree, PySpeciesTreeIter};
...
pub(crate) fn extract_extant_gene_tree(
    rec_tree: &RecTree,
) -> Result<FlatTree, crate::error::RustreeError> {
...
pub(crate) fn parse_distance_type(
    distance_type: &str,
) -> PyResult<crate::metric_functions::DistanceType> {
...
fn simulate_species_tree(
    n: usize,
    lambda_: f64,
    mu: f64,
    seed: Option<u64>,
) -> PyResult<PySpeciesTree> {
    if n == 0 {
    ...
    if mu < 0.0 {
        return Err(PyValueError::new_err(
            "Extinction rate must be non-negative",
        ));
    }
    if lambda_ <= mu {
        return Err(PyValueError::new_err(
    ...
    let (mut tree, _events) =
        simulate_bd_tree_bwd(n, lambda_, mu, &mut rng).map_err(PyValueError::new_err)?;
    tree.assign_depths();
...
pub(crate) fn parse_species_tree(newick_str: &str) -> PyResult<PySpeciesTree> {
    ...
    let newick_content = if Path::new(newick_str).exists() {
        fs::read_to_string(newick_str).map_err(|e| {
            PyValueError::new_err(format!("Failed to read file '{}': {}", newick_str, e))
        })?
    } else {
...
fn rustree(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_function(wrap_pyfunction!(
        training::create_training_sample_from_sim,
        m
    )?)?;
    ...
    m.add_function(wrap_pyfunction!(
        reconciliation::compare_reconciliations,
        m
    )?)?;
    m.add_function(wrap_pyfunction!(
        reconciliation::compare_reconciliations_multi,
        m
    )?)?;
