...
//!
//! All functions return `Result<_, String>` so callers can convert
//! to their language-specific error types (PyResult, extendr Result).
...
pub fn validate_dtl_rates(lambda_d: f64, lambda_t: f64, lambda_l: f64) -> Result<(), String> {
...
pub fn validate_replacement_transfer(replacement_transfer: Option<f64>) -> Result<(), String> {
    if let Some(p) = replacement_transfer {
        if !(0.0..=1.0).contains(&p) {
            return Err("replacement_transfer must be between 0.0 and 1.0".to_string());
        }
    }
    Ok(())
}
...
pub fn parse_distance_type(distance_type: &str) -> Result<DistanceType, String> {
...
pub fn extract_extant_gene_tree(rec_tree: &RecTree) -> Result<FlatTree, String> {
