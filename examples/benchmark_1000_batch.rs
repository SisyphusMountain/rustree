...
fn main() {
    ...
    println!(
        "  ✓ Generated {} species in {:.3}s\n",
        species_tree.nodes.len(),
        species_time.as_secs_f64()
    );
    ...
    let min_nodes = rec_trees
        .iter()
        .map(|t| t.gene_tree.nodes.len())
        .min()
        .unwrap();
    let max_nodes = rec_trees
        .iter()
        .map(|t| t.gene_tree.nodes.len())
        .max()
        .unwrap();
    ...
    println!("Timing:");
    println!(
        "  Species tree generation: {:.3}s",
        species_time.as_secs_f64()
    );
    println!(
        "  Gene tree simulations:   {:.3}s",
        simulation_time.as_secs_f64()
    );
    println!(
        "  Total time:              {:.3}s\n",
    ...
    println!(
        "Current (batch method): {:.3}s for 1000 trees",
        simulation_time.as_secs_f64()
    );
    println!(
        "Speedup: ~{:.1}x faster",
        4.8 / simulation_time.as_secs_f64()
    );
}
